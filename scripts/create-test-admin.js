...
async function createTestAdmin() {
  try {
    ...
    console.log("✅ Utilisateur admin de test créé avec succès :", result);
    await client.close();
  } catch (error) {
    if (error.message?.includes("User already exists") || error.message?.includes("EMAIL_ALREADY_USED")) {
      console.log("ℹ️  L'utilisateur admin de test existe déjà");
    } else {
      console.error("❌ Erreur lors de la création de l'admin de test :", error.message);
    }
  }
}
